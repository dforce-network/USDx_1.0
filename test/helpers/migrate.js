...
async function runConfig(
  ...
  async function verifyContracts() {
    var system;
    var runType;
    var configTimes = runConfig.hasOwnProperty("times")
      ? runConfig["times"]
      : runConfig["data"].length;

    var dfEngineIndex = 0;

    var dfEngineTimes = 0;
    while (dfEngineTimes < configTimes) {
      console.log(" dfEngine : " + (dfEngineTimes + 1) + "\n");

      dfEngineIndex = dfEngineTimes % runConfig["data"].length;
      runType = runConfig["data"][dfEngineIndex].hasOwnProperty("type")
        ? runConfig["data"][dfEngineIndex]["type"]
      ...
      var runTimes = 1;
      if (runConfig["data"][dfEngineIndex].hasOwnProperty("data")) {
        runTimes = runConfig["data"][dfEngineIndex]["data"].length;
      }
      if (runConfig["data"][dfEngineIndex].hasOwnProperty("times")) {
        runTimes = runConfig["data"][dfEngineIndex]["times"];
      }

      system = runConfig["data"][dfEngineIndex].hasOwnProperty("sys")
        ? runConfig["data"][dfEngineIndex]["sys"] - 1
      ...
      switch (true) {
        case runType == "deposit":
          while (condition < runTimes) {
            ...
            conditionIndex =
              condition % runConfig["data"][dfEngineIndex]["data"].length;
            if (runConfig["data"][dfEngineIndex].hasOwnProperty("data")) {
              // if (runConfig['data'][dfEngineIndex]['data'][conditionIndex].hasOwnProperty('sys')) {

              //     system = runConfig['data'][dfEngineIndex]['data'][conditionIndex]['sys'] - 1;
              //     dfProtocolMintingSection = await dfProtocolView[system].getMintingSection.call();
              //     tokenAddressList = dfProtocolMintingSection[0];
              // }

              if (
                runConfig["data"][dfEngineIndex]["data"][
                  conditionIndex
                ].hasOwnProperty("tokenAddress")
              ) {
                tokenAddress =
                  srcTokenAddress[
                    runConfig["data"][dfEngineIndex]["data"][conditionIndex][
                      "tokenAddress"
                    ] - 1
                  ];
              }
            ...
            assert.equal(
              tokenAddress,
              await wrapTokenContract[system][xTokenAddress].getSrcERC20.call()
            );
            ...
            for (let index = 0; index < srcTokenAddress.length; index++) {
              ...
              dfCollateralTokenBalance[xTokenAddress] = await wrapTokenContract[
                system
              ][xTokenAddress].balanceOf.call(dfCollateral[system].address);
            ...
            for (let index = 0; index < srcTokenAddress.length; index++) {
              ...
              dfPoolSrcTokenBalance[
                srcTokenAddress[index]
              ] = await srcTokenContract[srcTokenAddress[index]].balanceOf.call(
                dfPool[system].address
              );
        ...
        case runType == "updateSection":
          while (condition < runTimes) {
            ...
            for (let index = 0; index < srcTokenAddress.length; index++) {
              ...
              assert.equal(
                (
                  await wrapTokenContract[system][xTokenAddress].balanceOf.call(
                    dfPool[system].address
                  )
                ).toString(),
                (
        ...
        case runType == "setMinBurnAmount":
          while (condition < runTimes) {
            ...
            runData["amountNB"] = amountNB.toString();
            try {
              transactionData = await dfSetting[system].setDestroyThreshold(
                amountNB,
                {
                  from: owner,
                }
            ...
            assert.equal(
              (await dfProtocol[system].getDestroyThreshold.call()).toString(),
              amountNB.toString()
            );
            condition++;
          }
    ...
    if (lastConfig) {
      if (
        runConfig["data"][dfEngineIndex].hasOwnProperty("verify") &&
        runConfig["data"][dfEngineIndex]["verify"]
      ) {
        var wrapTokenList = [];
        for (
          let systemIndex = 0;
          ...
          for (let index = 0; index < wrapTokenList.length; index++) {
            ...
            dfTokenBalance = await dfStore[systemIndex].getResUSDXBalance.call(
              wrapTokenList[index]
            );
